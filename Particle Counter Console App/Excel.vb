...
Public Class Excel
    ...
    Public Function ReadCellstr(ByVal i As Integer, ByVal j As Integer) As String
        ...
        If range.Cells(i, j).Value IsNot Nothing Then
            'If column is "Sample Time"
            If j = 2 Then
                cellValue = ConvertFromDoubleToHHMMSSTT(range.Cells(i, 2).value)
                'If column is "Sample Date"
            ElseIf j = 1 Then
                Dim dateSample = range.Cells(i, 1).value.ToShortDateString
                cellValue = DateTime.ParseExact(dateSample, "dd/mm/yyyy", DateTimeFormatInfo.InvariantInfo).ToString("yyyy-mm-dd")
                'cellValue = DateTime.ParseExact(range.Cells(i, 1).value, "dd/mm/yyyy", DateTimeFormatInfo.InvariantInfo).ToString("yyyy-mm-dd")
            ElseIf j = 5 Then
                Dim dateSample = range.Cells(i, 1).value.ToShortDateString
                cellValue = DateTime.ParseExact(dateSample, "dd/mm/yyyy", DateTimeFormatInfo.InvariantInfo).ToString("yyyy-mm-dd")
                Dim lastupdatedTime = DateTime.ParseExact(range.Cells(i, j).value, "MM/dd/yyyy h:mm:ss tt", DateTimeFormatInfo.InvariantInfo).ToString("yyyy-MM-dd HH:mm:ss")
                cellValue = lastupdatedTime
            Else
